...
#22-05-2020 This file is for the Final Fine challenge.
...
ICTD_round1 <- function(data_bulk)
{
  ...
  aaa3<-compute_jaccard2(R1_filter_step1_results_new2[[4]],LM22_test_list2_plus_unique)
  ...
  for(i in 1:length(tg_ids))
  {
    ...
    if(sum(aaa1[,tg_ids[i]]>0.7)>0)
    {
      ...
      if(length(tg_id_c)>1)
      {
        ...
        {
          nn<-c(nn,length(intersect(LM22_test_list2_plus_unique[[tg_ids[i]]],R1_filter_step1_results_new1[[4]][[tg_id_c[j]]])))
        }
        if(max(nn)>0.4)
        {
    ...
    if((st==0)&(sum(aaa2[,tg_ids[i]]>0.7)>0))
    {
      ...
      if(length(tg_id_c)>1)
      {
        ...
        for(j in 1:length(tg_id_c))
        {
          nn<-c(nn,length(intersect(LM22_test_list2_plus_unique[[tg_ids[i]]],R1_filter_step1_results_new2[[4]][[tg_id_c[j]]])))
        }
        if(max(nn)>0.4)
        {
          tg_id_ccc<-tg_id_c[which(nn==max(nn))[1]]##################################
    ...
    if((st==0)&(sum(aaa3[,tg_ids[i]]>0.7)>0))
    {
      ...
      if(length(tg_id_c)>1)
      {
        ...
        for(j in 1:length(tg_id_c))
        {
          nn<-c(nn,length(intersect(LM22_test_list2_plus_unique[[tg_ids[i]]],R1_filter_step1_results_new3[[4]][[tg_id_c[j]]])))
        }
        if(max(nn)>0.4)
        {
          tg_id_ccc<-tg_id_c[which(nn==max(nn))[1]]###########################################
  ...
  for(i in 1:length(tg_markers))
  {
    ...
    if(length(tg_markers[[i]])==1)
    {
      tg_ccc<-intersect(rownames(data01),LM22_test_list2_plus_unique[[tg_ids[i]]])
      # print("debug 111")
      # print(i)
      # print(tg_ccc)
