...
compute_new_weight<-function(tg_list_c,tg_list2)
{
  ccc<-compute_jaccard2(tg_list_c,tg_list2)
  ccc<-apply(ccc,1,max)
  names(ccc)<-names(tg_list_c)
  return(ccc)
}

NMF_method1_test_version4_MM<-function(tg_list, data_ng, data_normalized, max_ES_cut=0.2,max_ES_cut2=0.4,tProp,NMF_RR=10)
{
  #Weight_c0<-rep(0,length(rownames(data_ng)))
  #names(Weight_c0)<-rownames(data_ng)
  #tg_ssss<-intersect(names(Weight_c0),names(TCGA_T_common_marker_weight))
  #Weight_c0[tg_ssss]<-TCGA_T_common_marker_weight[tg_ssss]
  ...
  Weight_list<-compute_new_weight(tg_list_c=tg_selected_R4_RR,MM_core_list)
  tg_I_id<-which(Weight_list==max(Weight_list))[1]
  ...
  ttt1<-list()
  kkk<-svd(X_I)
  if(mean(cor(kkk$v[,1],t(X_I)))<0)
  {
    kkk$v[,1]<--kkk$v[,1]
    kkk$u[,1]<--kkk$u[,1]
  }
  U1<-t(t(kkk$u[,1]))
  rownames(U1)<-rownames(X_I)
  V1<-t(t(kkk$v[,1]))
  rownames(V1)<-colnames(X_I)
  ttt1$U<-U1
  ttt1$V<-V1
  qnmf_result_c<-list()
  qnmf_result_c$X1<-X_I
  qnmf_result_c$U<-U1
  qnmf_result_c$V<-V1
  qnmf_result_c$extraOut<-0
  qnmf_result_c$ttt1<-ttt1
  while((ES_max>max_ES_cut)&(length(tg_id)>0))
  {
    ...
    tg_id_add<-tg_id[which(Weight_list[tg_id]==max(Weight_list[tg_id]))[1]]
    ...
    if(ES_base_c_temp[tg_id_add]>max_ES_cut2)
    {
...
ICTD_round1 <- function(data_bulk)
{
  ...
  tg_cc<-R1_filter_step1_results_new1[[4]]
  ccc<-compute_jaccard2(fff,tg_cc)
  tgs1<-which(ccc>=0.7)
  selected_Bases<-list()
  nn<-c()
  if(length(tgs1)>0)
  {
    for(j in 1:length(tgs1))
    {
      selected_Bases[[j]]<-tg_cc[[tgs1[j]]]
    }
    nn<-c(nn,names(R1_filter_step1_results_new1[[4]])[tgs1])
  }
  if(length(selected_Bases)<500)
  {
    tg_cc<-R1_filter_step1_results_new2[[4]]
    ccc<-compute_jaccard2(fff,tg_cc)
    tgs1<-which(ccc>=0.7)
    selected_Bases2<-list()
    if(length(tgs1)>0)
    {
      for(j in 1:length(tgs1))
      {
        selected_Bases2[[j]]<-tg_cc[[tgs1[j]]]
      }
      nn<-c(nn,names(R1_filter_step1_results_new2[[4]])[tgs1])
    }
    selected_Bases<-c(selected_Bases,selected_Bases2)
  }
  if(length(selected_Bases)<500)
  {
    tg_cc<-R1_filter_step1_results_new3[[4]]
    ccc<-compute_jaccard2(fff,tg_cc)
    tgs1<-which(ccc>=0.7)
    selected_Bases3<-list()
    if(length(tgs1)>0)
    {
      for(j in 1:length(tgs1))
      {
        selected_Bases3[[j]]<-tg_cc[[tgs1[j]]]
      }
      nn<-c(nn,names(R1_filter_step1_results_new3[[4]])[tgs1])
    }
    selected_Bases<-c(selected_Bases,selected_Bases3)
  }
  if(length(selected_Bases)>0)
  {	
    names(selected_Bases)<-paste(nn,1:length(selected_Bases),sep="_")
  }
  N<-0
  if(length(selected_Bases)>0)
  {
    kkk<-compute_jaccard2(selected_Bases,MM_core_list)
    tg_list_MM<-list()
    tgs1<-which(kkk[,1]>kkk[,2])
    tgs2<-which(kkk[,1]<kkk[,2])
    tg_id1<-0
    tg_id2<-0
    if(length(tgs1)>0)
    {
      if(max(kkk[tgs1,1])>0.3)
      {
        tg_id1<-tgs1[which(kkk[tgs1,1]==max(kkk[tgs1,1]))[1]]
      }
    }
    if(length(tgs2)>0)
    {
      if(max(kkk[tgs2,2])>0.3)
      {
        tg_id2<-tgs2[which(kkk[tgs2,2]==max(kkk[tgs2,2]))[1]]
      }
    }
    nn<-c()
    if(tg_id1>0)
    {
      N<-N+1
      tg_list_MM[[N]]<-selected_Bases[[tg_id1]]
      nn<-c(nn,"neutrophils")
    }
    if(tg_id2>0)
    {
      N<-N+1
      tg_list_MM[[N]]<-selected_Bases[[tg_id2]]
      nn<-c(nn,"monocytic.lineage")
    }
  }
  ...
  {
    tg_markers<-list()
    tg_markers[[1]]<-top5_SVD_cor(data01,LM22_test_list1_plus_unique_core[["neutrophils"]])
    tg_markers[[2]]<-top5_SVD_cor(data01,LM22_test_list1_plus_unique_core[["monocytic.lineage"]])
    names(tg_markers)<-c("neutrophils","monocytic.lineage")
  }
  if(N==1)
  {
    tg_markers<-list()
    tg_markers[[1]]<-tg_list_MM[[1]]
    ff<-setdiff(c("neutrophils","monocytic.lineage"),nn)
    tg_markers[[2]]<-top5_SVD_cor(data01,LM22_test_list1_plus_unique_core[[ff]])
    names(tg_markers)<-c(nn,ff)
  }
  if(N==2)
  {
    names(tg_list_MM)<-nn
  ...
  LM22_test_list1_plus_unique_core[[4]]
  fff<-list()
  fff[[1]]<-unique(c(LM22_test_list1_plus[[2]],LM22_test_list1_plus[[3]],LM22_test_list1_plus[[4]]))
  tg_cc<-R1_filter_step1_results_new1[[4]]
  ccc<-compute_jaccard2(fff,tg_cc)
  tgs1<-which(ccc>=0.5)
  selected_Bases<-list()
  ...
  if(length(selected_Bases)>0)
  {
    ...
    tgs4<-which(kkk[,4]>apply(kkk[,-4],1,max))
    tgs5<-which(kkk[,5]>apply(kkk[,-5],1,max))
    tg_id1<-0
    tg_id2<-0
    tg_id3<-0
    tg_id4<-0
    ...
    if(length(tgs3)>0)
    {
      if(max(kkk[tgs3,3])>0.3)
      {
        tg_id3<-tgs3[which(kkk[tgs3,3]==max(kkk[tgs3,3]))[1]]
      }
    }
    if(length(tgs4)>0)
    {
      if(max(kkk[tgs4,4])>0.3)
      {
        tg_id4<-tgs4[which(kkk[tgs4,4]==max(kkk[tgs4,4]))[1]]
      }
    }
    if(length(tgs5)>0)
    {
      if(max(kkk[tgs5,5])>0.3)
      {
        tg_id5<-tgs5[which(kkk[tgs5,5]==max(kkk[tgs5,5]))[1]]
      }
    }
    nn<-c()
    if(tg_id1>0)
    {
      N<-N+1
      tg_list_TT[[N]]<-selected_Bases[[tg_id1]]
      nn<-c(nn,"Tcells")
    }
    if(tg_id2>0)
    {
      N<-N+1
      tg_list_TT[[N]]<-selected_Bases[[tg_id2]]
      nn<-c(nn,"GZs")
    }
    if(tg_id3>0)
    {
      N<-N+1
      tg_list_TT[[N]]<-selected_Bases[[tg_id3]]
      nn<-c(nn,"CD4.T.cells")
    }
    if(tg_id4>0)
    {
      N<-N+1
      tg_list_TT[[N]]<-selected_Bases[[tg_id4]]
      nn<-c(nn,"CD8.T.cells")
    }
    if(tg_id5>0)
    {
      N<-N+1
      tg_list_TT[[N]]<-selected_Bases[[tg_id5]]
      nn<-c(nn,"NK.cells")
    }
  }
  
  
  tgs<-setdiff(c("Tcells","GZs","CD4.T.cells","CD8.T.cells","NK.cells"),nn)
  if(length(tgs)>0)
  {
    for(i in 1:length(tgs))
    {
      N<-N+1
      tg_list_TT[[N]]<-top5_SVD_cor(data01,TT_core2[[tgs[[i]]]])
      nn<-c(nn,tgs[[i]])
    }
  }
  names(tg_list_TT)<-nn
  ...
  for(i in 1:length(tg_list_TT))
  {
    tg_genes_all<-c(tg_genes_all,tg_list_TT[[i]])
  }
  
  tg_genes_all<-unique(tg_genes_all)
  label_matrix<-matrix(0,length(tg_genes_all),3)
  rownames(label_matrix)<-tg_genes_all
  colnames(label_matrix)<-c("CD4.T.cells","CD8.T.cells","NK.cells")
  label_matrix[tg_list_TT[["Tcells"]],c(1:2)]<-1
  label_matrix[tg_list_TT[["GZs"]],c(2:3)]<-1
  label_matrix[tg_list_TT[["CD4.T.cells"]],1]<-1
  label_matrix[tg_list_TT[["CD8.T.cells"]],2]<-1
  label_matrix[tg_list_TT[["NK.cells"]],3]<-1
  
  preP<-matrix(0,3,ncol(data0))
  rownames(preP)<-colnames(label_matrix)
  colnames(preP)<-colnames(data0)
  SSS<-c(1,1,1)
  names(SSS)<-colnames(label_matrix)
  NMF_self_cT<-list()
  NMF_self_cT[[1]]<-data0[tg_genes_all,]
  NMF_self_cT[[2]]<-label_matrix
  NMF_self_cT[[3]]<-label_matrix
  NMF_self_cT[[4]]<-SSS
